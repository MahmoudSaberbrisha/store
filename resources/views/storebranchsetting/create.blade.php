...
@section('content')
    ...
    <form action="{{ route('storebranchsetting.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="title" name="title" maxlength="15" value="{{ old('title') }}">
        </div>
        <div class="mb-3">
            <label for="br_code" class="form-label">Branch Code</label>
            <select class="form-select" id="br_code" name="br_code">
                <option value="" {{ old('br_code') == '' ? 'selected' : '' }}>None</option>
        ...
        <div class="mb-3">
            ...
            <select class="form-select" id="from_id" name="from_id">
                <option value="" {{ old('from_id') == '' ? 'selected' : '' }}>None</option>
